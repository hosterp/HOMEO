...
class CreditLimitCustomer(models.Model):
    ...
    @api.onchange('days_credit_limit')
    def compute_credit_end_date(self):
        for rec in self:
            if rec.days_credit_limit:
                last_payment = self.env['partner.payment'].search(
                    [('partner_id', '=', rec.id), ('balance_amount', '=', 0)],
                    order='id desc', limit=1)
